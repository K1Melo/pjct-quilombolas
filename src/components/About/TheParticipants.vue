<template>
    <div class="participants">
        <div class="participants-container">
            <div class="tools">
                <div class="participants">
                    <img src="../../assets/logo.png" alt="">
                    <p class="nome">Nome</p>
                    <svg id="github" @click="openGitHubParticipant('https://github.com/K1Melo')" xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                        viewBox="0 0 16 16">
                        <path
                            d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59c.4.07.55-.17.55-.38c0-.19-.01-.82-.01-1.49c-2.01.37-2.53-.49-2.69-.94c-.09-.23-.48-.94-.82-1.13c-.28-.15-.68-.52-.01-.53c.63-.01 1.08.58 1.23.82c.72 1.21 1.87.87 2.33.66c.07-.52.28-.87.51-1.07c-1.78-.2-3.64-.89-3.64-3.95c0-.87.31-1.59.82-2.15c-.08-.2-.36-1.02.08-2.12c0 0 .67-.21 2.2.82c.64-.18 1.32-.27 2-.27c.68 0 1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82c.44 1.1.16 1.92.08 2.12c.51.56.82 1.27.82 2.15c0 3.07-1.87 3.75-3.65 3.95c.29.25.54.73.54 1.48c0 1.07-.01 1.93-.01 2.2c0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                    </svg>
                </div>
                <div class="participants">
                    <img src="../../assets/logo.png" alt="">
                    <p class="nome">Nome</p>
                    <svg id="github" @click="openGitHubParticipant('https://github.com/K1Melo')" xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                        viewBox="0 0 16 16">
                        <path
                            d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59c.4.07.55-.17.55-.38c0-.19-.01-.82-.01-1.49c-2.01.37-2.53-.49-2.69-.94c-.09-.23-.48-.94-.82-1.13c-.28-.15-.68-.52-.01-.53c.63-.01 1.08.58 1.23.82c.72 1.21 1.87.87 2.33.66c.07-.52.28-.87.51-1.07c-1.78-.2-3.64-.89-3.64-3.95c0-.87.31-1.59.82-2.15c-.08-.2-.36-1.02.08-2.12c0 0 .67-.21 2.2.82c.64-.18 1.32-.27 2-.27c.68 0 1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82c.44 1.1.16 1.92.08 2.12c.51.56.82 1.27.82 2.15c0 3.07-1.87 3.75-3.65 3.95c.29.25.54.73.54 1.48c0 1.07-.01 1.93-.01 2.2c0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                    </svg>
                    
                </div>
                <div class="participants">
                    <img src="../../assets/logo.png" alt="">
                    <p class="nome">Nome</p>
                    <svg id="github" @click="openGitHubParticipant('https://github.com/K1Melo')" xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                        viewBox="0 0 16 16">
                        <path
                            d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59c.4.07.55-.17.55-.38c0-.19-.01-.82-.01-1.49c-2.01.37-2.53-.49-2.69-.94c-.09-.23-.48-.94-.82-1.13c-.28-.15-.68-.52-.01-.53c.63-.01 1.08.58 1.23.82c.72 1.21 1.87.87 2.33.66c.07-.52.28-.87.51-1.07c-1.78-.2-3.64-.89-3.64-3.95c0-.87.31-1.59.82-2.15c-.08-.2-.36-1.02.08-2.12c0 0 .67-.21 2.2.82c.64-.18 1.32-.27 2-.27c.68 0 1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82c.44 1.1.16 1.92.08 2.12c.51.56.82 1.27.82 2.15c0 3.07-1.87 3.75-3.65 3.95c.29.25.54.73.54 1.48c0 1.07-.01 1.93-.01 2.2c0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                    </svg>
                    
                </div>
                <div class="participants">
                    <img src="../../assets/logo.png" alt="">
                    <p class="nome">Nome</p>
                    <svg id="github" @click="openGitHubParticipant('https://github.com/K1Melo')" xmlns="http://www.w3.org/2000/svg" width="30" height="30"
                        viewBox="0 0 16 16">
                        <path
                            d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59c.4.07.55-.17.55-.38c0-.19-.01-.82-.01-1.49c-2.01.37-2.53-.49-2.69-.94c-.09-.23-.48-.94-.82-1.13c-.28-.15-.68-.52-.01-.53c.63-.01 1.08.58 1.23.82c.72 1.21 1.87.87 2.33.66c.07-.52.28-.87.51-1.07c-1.78-.2-3.64-.89-3.64-3.95c0-.87.31-1.59.82-2.15c-.08-.2-.36-1.02.08-2.12c0 0 .67-.21 2.2.82c.64-.18 1.32-.27 2-.27c.68 0 1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82c.44 1.1.16 1.92.08 2.12c.51.56.82 1.27.82 2.15c0 3.07-1.87 3.75-3.65 3.95c.29.25.54.73.54 1.48c0 1.07-.01 1.93-.01 2.2c0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z" />
                    </svg>
                    
                </div>
                <div class="participants">
                    <img src="../../assets/logo.png" alt="">
                    <p class="nome">Nome</p>

                </div>
            </div>
            <div class="participants-project">
                <div class="pjct-logo">
                    <img src="../../assets/logo.png" alt="">
                </div>
                <div class="pjct-participants"></div>
            </div>
        </div>
    </div>
</template>

<script>

export default {
    methods: {
        openGitHubParticipant(link) {
            window.open(link);

        }
    }
}

</script>

<style lang="less" scoped>
.participants {
    height: 100vh;
    width: 100%;
    display: flex;
    align-items: center;
    justify-content: center;
}

.participants-container {
    width: 60%;
    height: 80%;
    background: rgba(255, 255, 255, 0.687);
    box-shadow: 0 8px 32px 0 rgba(147, 147, 147, 0.37);

    border-radius: 20px;
    border: 1px solid rgba(255, 255, 255, 0.18);

    overflow: hidden;

    display: flex;
    align-items: center;
}

.tools {
    width: 70%;
    height: 100%;

    .participants {
        height: 20%;
        background-color: aqua;
        border: 1px solid black;
        display: flex;
        justify-content: flex-start;
        align-items: center;

        img {
            width: 10%;
        }

        p {
            margin: 0;
        }
    }
}

.participants-project {
    width: 30%;
    height: 100%;
    background-color: rgba(0, 0, 0, 0.662);

    display: flex;
    align-items: center;
    justify-content: center;
    flex-direction: column;
}

.pjct-logo {
    display: flex;
    align-items: center;
    justify-content: center;
    img {
        width: 30%;
    }
}   

#github {

cursor: pointer;

}

@media only screen and (max-width: 820px) {
    .participants-container {
        width: 80%;
        height: 80%
    }
}


@media only screen and (max-width: 650px) {
    .participants-container {
        width: 80%;
        height: 80%
    }
}

@media only screen and (max-width: 300px) {
    .participants-container {
        width: 80%;
        height: 80%
    }
}


</style>